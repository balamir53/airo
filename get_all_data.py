...
query = f'select substring(addedtodownloadqueue ,0,11) addedtodownloadqueue, deptime, recordroute, flightno, min(price) price FROM airo.newview group by deptime, flightno, substring(addedtodownloadqueue ,0,11), recordroute'
count = 0
...
file_path = folder_path + '/groupby-all%s.pkl' % (count)
